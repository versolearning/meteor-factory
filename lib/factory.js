...
Factory._create = function(name, doc) {
  ...
  if (factory.collection.mutate && factory.collection.mutate.insert) {
    ...
    // Send notifications synchronously so that the tests in `notifications-tests.js` will work properly
    if (factory.collection._name === 'flips')
      options.sendNotificationsSynchronously = true;

    var result = factory.collection.mutate.insert(doc, options);
    
    if (result.ok) {
      insertId = result._id;
    } else {
      throw new Error('Factory: Invalid Document (' + factory.collection._name + ') ' +
        EJSON.stringify(doc) + ' Result: ' + EJSON.stringify(result));
    }
  } else {
